...
export default function SidebarNav() {
  ...
  const canPollClientChatsUnread = !loading && hasPermission("client_chats.menu");
  useEffect(() => {
    if (!canPollClientChatsUnread) {
      setChatUnread(0);
      return;
    }
    let cancelled = false;
    ...
    <nav className="px-2 pb-3">
      <div className="space-y-2">
        {orderedItems.map((item) => {
          ...
          const badge =
            item.href === "/app/client-chats" && chatUnread > 0 ? chatUnread : undefined;
          return (
            <RailItem
              key={item.href}
              href={item.href}
  ...
  return (
    ...
    >
      ...
      <span className="relative">
        ...
        {badge != null && badge > 0 && (
          <span
            className="absolute -top-1.5 -right-1.5 flex h-5 min-w-5 items-center justify-center rounded-full bg-red-500 px-1 text-[10px] font-bold text-white ring-2 ring-white"
...
function IconCallCenter() {
  return (
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
      ...
      />
      <path
        d="M14.5 2a6.5 6.5 0 0 1 6.5 6.5"
        stroke="currentColor"
        strokeWidth="1.8"
        strokeLinecap="round"
      />
      <path
        d="M14.5 6A2.5 2.5 0 0 1 17 8.5"
        stroke="currentColor"
        strokeWidth="1.8"
        strokeLinecap="round"
      />
