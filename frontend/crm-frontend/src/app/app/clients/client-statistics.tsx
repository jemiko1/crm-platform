      ...
      >
        <div className="sticky top-0 z-10 bg-white border-b border-zinc-200 px-6 py-5">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-xl font-semibold text-zinc-900">Monthly Client Statistics</h2>
              <p className="mt-1 text-sm text-zinc-600">View clients added per month</p>
            </div>
            <button type="button" onClick={onClose} className="rounded-full p-2 text-zinc-500 hover:bg-zinc-100 hover:text-zinc-900 transition-colors">
              <IconClose />
            </button>
          </div>
          ...
          <div className="mt-4 flex items-center gap-3">
            <button
              ...
              onClick={() => {
                ...
                if (i < availableYears.length - 1) setSelectedYear(availableYears[i + 1]);
              }}
            ...
            <button
              ...
              onClick={() => {
                const i = availableYears.indexOf(selectedYear);
                if (i > 0) setSelectedYear(availableYears[i - 1]);
              }}
        ...
        <div className="overflow-y-auto max-h-[calc(90vh-180px)] px-6 py-4">
          {availableYears.length === 0 ? (
            <div className="py-12 text-center text-sm text-zinc-600">No client data available yet</div>
          ) : (
            <div className="space-y-2">
              {monthNames.map((monthName, index) => {
                ...
                return (
                  <div
                    key={monthName}
                    className={["flex items-center justify-between rounded-xl px-4 py-3", hasData ? "bg-teal-50 ring-1 ring-teal-200" : "bg-zinc-50"].join(" ")}
                  >
                    <span className={["text-sm font-medium", hasData ? "text-zinc-900" : "text-zinc-500"].join(" ")}>{monthName}</span>
...
export default function ClientStatistics({
  ...
  if (error) {
    return (
      <div className="mb-4 rounded-lg bg-amber-50 p-3 ring-1 ring-amber-200 md:mb-6 md:rounded-2xl md:p-4" data-testid="client-statistics">
  ...
  return (
    <>
      <div
        ...
        data-testid="client-statistics"
      >
        <StatBox
          title="Total Clients"
          value={loading ? "..." : statistics?.totalClientsCount ?? 0}
          subtitle="All time"
          loading={loading}
          variant="total"
          icon={<IconClients />}
...
function IconTrend() {
  return (
    <svg className="h-4 w-4 md:h-5 md:w-5" viewBox="0 0 24 24" fill="none">
      <path d="M3 17l6-6 4 4 8-12" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
      <path d="M17 17H21V13" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}
...
function IconChart() {
  return (
    <svg className="h-4 w-4 md:h-5 md:w-5" viewBox="0 0 24 24" fill="none">
      ...
      <path d="M7 14l4-4 4 2 6-6" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}
