...
>
> **Last updated:** 2026-05-01 (`/cleanup` slash command merged as PR #333; live-monitoring agent-status fixes + softphone switch-user presence cleanup in flight on `feature/softphone-bug-fixes`)
...
|---|---|---|---|
| (next) | `fix(telephony): live agent status — heartbeat-driven online/offline + queue-30 filter + switch-user presence` | 🔨 open, pending review | **Three bugs in `/app/call-center/live` Agents panel.** **(1)** Operators showed "Offline" until first call event — `currentState` was derived only from the in-memory AMI state map, which never flips on pure SIP registration. Fix in `telephony-live.service.ts::applySipPresenceToCurrentState()`: SIP heartbeat (`TelephonyExtension.sipRegistered + sipLastSeenAt`, freshness 90s mirroring `AgentPresenceService.STALE_AFTER_MS`) is the source of truth for online/offline; the in-memory map is only an activity overlay (ON_CALL wins, OFFLINE gets promoted to IDLE when heartbeat fresh, stale forces OFFLINE regardless). **(2)** Agents panel listed managers/IT/admins. New `getQualifyingAgentUserIds()` filters to extensions whose linked user has Position with `RoleGroup.code === 'CALL_CENTER'` AND a `PositionQueueRule` for queue named `'30'` (the main inbound queue). Permission to view the page unchanged — only the grid contents narrow. Throttled warning log if filter empties everyone (catches future "queue 30 was renamed in FreePBX" regressions). **(3)** After softphone user-switch (e.g. bela 101 → sofo 102), bela kept showing "Idle" because the renderer's unregister POST fired under sofo's new JWT and the backend's extension-mismatch guard rejected it. Fix in `crm-phone/src/main/local-server.ts /switch-user`: post `state=unregistered` for the outgoing user using their JWT BEFORE `setSession(data)`. Best-effort try/catch — network failure must not block the swap (90s stale sweep is the safety net). 7 new unit tests; 669/669 pass; backend typecheck + lint clean; softphone main typecheck clean (pre-existing `ws` types error unrelated). Reviewed by `code-reviewer` — 0 critical, 2 warnings (empty-filter warning log + pool-rows regression test) addressed pre-push. New Silent Override Risk #34 added. |
| #333 | `feat(claude): /cleanup command for opt-in branch + stash audit` | ✅ merged 2026-05-01 | **Standalone follow-up to PR #332.** Adds a single new slash command `/cleanup` (`.claude/commands/cleanup.md`) for on-demand audit of (a) local branches whose PR is confirmed merged on GitHub, (b) git stashes, (c) local branches with no PR ever (informational). Explicitly NOT auto-invoked — Jemiko runs many parallel Claude Code sessions in this repo, so any auto-cleanup in `/sync` or `/done` could delete a branch another session is working from. The command is fully manual, presents each block with confirmation prompts, never touches current branch / master / worktree-attached branches / remote refs, drops stashes only with per-item or named-batch confirmation, and uses `git branch -d` (not `-D`) so squash-merged branches require explicit force-confirmation. Cheatsheet `docs/CLAUDE_CODE_CHEATSHEET.md` updated to list `/cleanup` in the slash-command table. |
