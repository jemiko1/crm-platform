...
|---|---|---|---|
| (next) | `fix(softphone): v1.12.2 — auto-update from GitHub releases as single source of truth` | 🔨 open, pending review | **Permanent fix for the recurring "Check for Updates says 'up to date' even after I shipped a release" complaint.** Auto-updater was using `provider: generic, url: https://crm28.asg.ge/downloads/phone` — a static nginx path on the production VM. Every release required SCPing `*.exe + blockmap + latest.yml` to that path AFTER `gh release create`, and that manual step kept getting skipped. Field symptom that triggered this PR: founder merged v1.12.1 (cert-pin), clicked Check for Updates on his v1.12.0 softphone, got "up to date" because the VM still served `version: 1.12.0`. Fix: `electron-builder.yml` `publish.provider` switched from `generic` to `github` (`owner: jemiko1, repo: crm-platform, releaseType: release, vPrefixedTagName: true`); the redundant `autoUpdater.setFeedURL(...)` call in `auto-updater.ts` removed so the embedded `app-update.yml` is the single source of truth (no code-vs-yml drift possible). Repo is public so no auth tokens at runtime. From v1.12.3 onward `gh release create` is the single action that ships a build. v1.12.2 was bridged ONE LAST TIME to the VM path so existing v1.12.0/v1.12.1 operators (with the old baked-in feed) auto-update. Stale `crm-phone/UPDATE.md` collapsed to a one-line redirect to the new `docs/SOFTPHONE_RELEASE_PROCEDURE.md`. New CLAUDE.md Silent Override Risk #30 captures the no-`setFeedURL`-override + no-split-config rules. Stray esbuild artifacts (`/index.js`, `/preload.js`) added to `crm-phone/.gitignore`. Reviewed by `code-reviewer` — caught the stale UPDATE.md (would have contradicted the new procedure) and the stray `index.js` artifact. |
