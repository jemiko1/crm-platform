...
>
> **Last updated:** 2026-04-27 (softphone v1.12.0 — Staff directory tab + DND in Settings + UI refine in flight; queue-member-mariadb-sync also still open)
...
|---|---|---|---|
| (next) | `fix(softphone): v1.12.3 — auto-update via single-command release script (private-repo-ready)` | 🔨 open, pending review | **Permanent fix for the recurring "Check for Updates says 'up to date' even after I shipped a release" complaint, with the constraint that the GitHub repo is going private soon.** Three flips in three days landed at the right answer: pre-1.12.2 used `provider: generic` + manual SCP that kept getting skipped → operators stuck; PR #309 (closed) tried `provider: github` against the public repo → would have broken auto-update the moment the repo flipped private; this PR reverts to `provider: generic` BUT bundles the SCP into a single `pnpm run release` command so the "skip the SCP step" failure mode is structurally eliminated (it IS the same step as the build). New `crm-phone/scripts/release.sh` + `release.ps1`: build + verify `latest.yml`/installer name match + scp to VM + re-fetch the public URL to verify nginx now serves the new version + (optional) GitHub release for changelog audit trail. Three release-time integrity checks the old workflow had none of. `setFeedURL()` removed from `auto-updater.ts` — embedded `app-update.yml` is the single source of truth (no code-vs-yml drift). `artifactName: CRM28-Phone-Setup-${version}.${ext}` pinned in `electron-builder.yml` so file-on-disk, URL in `latest.yml`, and upload target are all identical (latent v1.12.1 bug). v1.12.3 was published BOTH via `pnpm run release` (uploads to VM AND creates a GitHub release as a one-time bridge) so v1.12.2 operators with the old GitHub-baked feed get the upgrade BEFORE the repo can flip private. Stale `crm-phone/UPDATE.md` collapsed to one-line redirect. Stray esbuild artifacts (`/index.js`, `/preload.js`) added to `.gitignore`. New CLAUDE.md Silent Override Risk #30. PR #309 closed (its v1.12.2 GitHub-feed approach is the wrong direction given the privacy constraint). |
