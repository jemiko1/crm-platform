...
|---|---|---|---|
| (next) | `fix(softphone): v1.10.1 — auto-updater retry crash (pnpm hoist + override)` | 🔨 open, pending review | Hotfix for `Update error: (0, builder_util_runtime_1.retry) is not a function` on v1.9.0/v1.10.0. Root cause: pnpm let `builder-util-runtime@9.2.4` (from electron-builder) hoist over `electron-updater@6.8.3`'s required `9.5.1`, which then got bundled into the asar without the `retry` export. Fix: `.npmrc` with `shamefully-hoist=true` + `pnpm.overrides` pinning `builder-util-runtime: 9.5.1`. Verified asar contains 9.5.1 + `exports.retry`. Silent Override Risk #22 documents both levers. |
