...
|---|---|---|---|
| (next) | `fix(telephony): asterisk-sync hard-deletes CRM extensions removed from FreePBX` | 🔨 open, pending review | **Closes the "ghost extensions" gap.** Before this fix, the asterisk-sync cron at `asterisk-sync.service.ts:syncExtensions()` was strictly additive — it created/updated CRM `TelephonyExtension` rows from FreePBX endpoints but never removed CRM rows whose extension had disappeared from FreePBX. Result: admin deletes ext 200 in FreePBX GUI, CRM admin Telephony Extensions page still shows 200 forever, can offer to link an operator to a non-existent extension. **Fix:** new cleanup pass after the add/update loop hard-deletes any CRM row whose `extension` is not in the live FreePBX endpoint set. Hard-delete is correct because every history table (`CallSession.assignedExtension`, `CallLeg.extension`, `OperatorBreakSession.extension`) stores the extension as a string snapshot, not a FK — stats and reports attribute through `assignedUserId`, so deleting the TelephonyExtension row preserves zero history loss. Resurrection is automatic: if FreePBX recreates the same number, the existing add/update branch produces a fresh CRM row on the next sync. **Two safety guards:** (1) cleanup never runs if AMI fetch failed (handled by existing early-return at `syncExtensions` top); (2) sanity threshold — refuse to delete more than 50% of currently-active CRM rows in one tick, log warning instead. Catches sync-bug / FreePBX-empty-by-mistake scenarios before nuking the table. New Silent Override Risk #30 added. 4 new unit tests (happy path, AMI-fetch-fail, threshold-bail, no-stale-rows). Backend-only; no schema change, no UI change, no softphone version bump. Reviewed by `code-reviewer`. |
