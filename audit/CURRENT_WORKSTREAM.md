...
|---|---|---|---|
| (next) | `feat(ui): Break + DND manager UI — Breaks tab + live-monitor socket events` | 🔨 open, pending review | New `/app/call-center/breaks` page with live-on-break list (1s ticker) + paginated history with system-ended vs operator-ended chips. `operator:break:started/:ended` socket events emitted from `OperatorBreakService` → `TelephonyGateway` → `dashboard` + `agent:<id>` rooms. Callback pattern mirrors `AgentPresenceService.onStaleFlipped`. i18n en + ka keys added. |
