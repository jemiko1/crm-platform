...
|---|---|---|---|
| #330 | `fix(reports): inclusive end-date filter across all reporting and stats services` | 🔨 open, pending review | **Bug:** selecting end date "Apr 30" in any report/stats page excluded all records from Apr 30. `new Date("YYYY-MM-DD")` parses as UTC midnight; used directly as `lte` it excludes every record created after 00:00 UTC on that day. **Fix:** cap upper boundary at `setUTCHours(23, 59, 59, 999)` in 6 services: `telephony-calls.service.ts`, `telephony-quality.service.ts`, `call-reports.service.ts` (was `setHours` → `setUTCHours`), `operator-break.controller.ts`, `clientchats-analytics.service.ts`, and `telephony-stats.service.ts` (added `endOfDay=true` flag to `parseDate()` private helper — covers all 6 stats endpoints including call-center dashboard). 106 suites, 663 tests pass. Code-reviewer run; W1 (stats) fixed in same PR. |
