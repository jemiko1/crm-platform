...
|---|---|---|---|
| (next) | `feat(telephony): DND feature backend — service + controller + logout hook + tests` | 🔨 open, pending review | `OperatorDndService` wraps AMI `QueuePause` (no Queue field → all queues). No DB column — state in Asterisk + cached in `TelephonyStateManager` (presence PAUSED). Three endpoints at `/v1/telephony/dnd/*` (all operator-own). Auto-disable on logout via best-effort cookie/JWT verify in `auth.controller.logout`. 12 unit tests pass. |
