...
|---|---|---|---|
| (next) | `fix(telephony): remove 50% sanity threshold from asterisk-sync cleanup` | 🔨 open, pending review | Follow-up to PR #319 (sync cleanup). The 50% mass-delete threshold added in #319 actively blocks admin's normal workflow — wiping all extensions in FreePBX and recreating them in bulk is a routine operation, and the threshold refuses to clean up after it. Removed entirely. Cleanup now runs unconditionally when there are stale rows, gated only by the existing "skip cleanup if AMI fetch failed" early-return. Updated CLAUDE.md Risk #30 to reflect the simpler design ("admin trust beats false-positive friction"). Threshold-bail unit test replaced with a "trust mass-delete" test that verifies the cleanup deletes 4 of 5 stale rows. Reviewed by `code-reviewer`. |
