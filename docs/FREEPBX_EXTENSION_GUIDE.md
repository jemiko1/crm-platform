...
> The Electron softphone connects via WSS (WebSocket Secure) to
> `wss://pbx.asg.ge:8089/ws`. The softphone trusts the PBX cert via a pinned
> SHA-256 SPKI hash baked into the build (see `crm-phone/src/main/pbx-cert-pin.ts`
> and `docs/SOFTPHONE_CERT_PINNING.md`), so the FreePBX self-signed cert is
> fine — but the extension also needs a specific WebRTC config or the operator
> can never register, no matter how good their password is. **That is what
> this guide is about.**
