...
29. **Softphone WSS depends on a public-CA cert at `pbx.asg.ge:8089`, manually renewed every ~60 days via DNS-01** (2026-04-28 incident — entire call center went offline). Softphone v1.10.x had a `setCertificateVerifyProc` blanket bypass trusting any cert; PR #292 (security audit) removed it correctly but didn't pair with a real cert deploy on the PBX, so when operators upgraded to v1.11.x they all failed to register against the still-self-signed PBX cert. Fix: ZeroSSL public cert via acme.sh DNS-01 manual mode, FQDN `pbx.asg.ge` (DNS A → 5.10.34.153), installed at `/etc/asterisk/keys/integration/{certificate.pem,webserver.key}` with `chmod 600 chown asterisk:asterisk`, hot-reloaded via `asterisk -rx "module reload http"`. **Renewal is manual** — `asg.ge` DNS provider has no API, and the PBX's network blocks LE HTTP-01 from the public internet, so DNS-01 manual is the only viable path. Owner adds a fresh `_acme-challenge.pbx` TXT record every ~60 days. The bridge-monitor (`/api/pbx-cert`) checks expiry hourly and shows a header badge in the dashboard at < 21 days (warning, amber) and < 7 days (critical, red). **Lesson:** when removing a security workaround, always identify what it was hiding and pair the removal with a fix on the underlying issue. PR #292's fix was technically correct but operationally incomplete — the same pattern can repeat elsewhere if reviewers don't ask "what assumption is this workaround making?". Renewal procedure: `docs/PBX_TLS_CERT_SOP.md`. Extension creation procedure: `docs/FREEPBX_EXTENSION_GUIDE.md`.
