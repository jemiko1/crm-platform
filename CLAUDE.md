...
30. **`asterisk-sync` cleanup hard-deletes CRM extensions removed from FreePBX, gated by a 50% sanity threshold** (telephony-sync-cleanup-stale-extensions PR) — When extensions disappear from FreePBX (admin deletes via GUI, Bulk Handler, or fwconsole), the next `syncExtensions` tick hard-deletes the matching `TelephonyExtension` rows in CRM. This is correct and safe because every history table — `CallSession.assignedExtension`, `CallLeg.extension`, `OperatorBreakSession.extension` — stores the extension as a string snapshot, not a foreign key. Stats and call logs attribute through `assignedUserId` (the durable User identity), so deleting the TelephonyExtension row destroys zero history. If the extension is later recreated in FreePBX with the same number, the existing add/update branch produces a fresh CRM row and admin re-links the operator. Two safety guards: (a) **cleanup only runs after a successful AMI fetch** — if `fetchEndpointsViaCli()` threw, the early return at the top of `syncExtensions` skips cleanup entirely, so a transient AMI outage cannot nuke the table; (b) **sanity threshold of 50%** — if the cleanup would mark more than half of currently-active CRM rows for deletion in a single tick, it bails with a warning log instead. This catches "FreePBX returned an empty list because of a sync bug" before it cascades. Admin can still trigger an intentional mass-delete via the "Sync Now" button in Telephony Extensions admin UI (the threshold check still applies — for a true wipe, admin must delete from FreePBX first, then call Sync Now). **Do NOT add a soft-delete (`deletedAt`) column** — the schema has no FK pressure that requires it, and a soft-delete tombstone introduces a new "stale row" class that needs its own cleanup. Hard-delete + threshold guard is the simpler durable design.
