...
30. **Softphone auto-update feed AND new-user download both live on the VM, not GitHub — `pnpm run release` is the single atomic command that refreshes BOTH** (softphone v1.12.3, `crm-phone/scripts/release.{sh,ps1}` + `crm-phone/electron-builder.yml` + frontend `header-settings.tsx` / `phone-mismatch-banner.tsx`) — Three flips in three days landed at the right answer: pre-1.12.2 used `provider: generic` + a separate manual SCP step that kept getting skipped → operators stuck on "up to date"; v1.12.2 switched to `provider: github` against the public repo to make `gh release create` the only step → worked but blocked the plan to make the repo private; v1.12.3 reverted to `provider: generic` BUT bundled the SCP into a single `pnpm run release` command that builds + uploads + refreshes the version-less stable copy + verifies in one atomic action → SCP can't be skipped because it IS the release step. **The release contract — four URLs that MUST all serve the same version after the script returns success:** `https://crm28.asg.ge/downloads/phone/latest.yml` (auto-update version-check), `.../CRM28-Phone-Setup-X.Y.Z.exe` (auto-update binary), `.../CRM28-Phone-Setup-X.Y.Z.exe.blockmap` (differential), and `.../CRM28-Phone-Setup.exe` (the version-less stable copy that the frontend "Download Phone App" buttons in settings dropdown + bridge-unreachable banner serve to NEW users). The first three are uploaded by `scp`; the fourth is refreshed by an `ssh Copy-Item -Force` step in the same script. The script's HEAD-check on the stable URL's `Content-Length` against the just-built installer catches the worst class of bug — auto-update and website-download serving different versions, where existing users get vX but new users install v(X-1). The `electron-builder.yml` `publish.url` is `https://crm28.asg.ge/downloads/phone`; `setFeedURL()` in `auto-updater.ts` is REMOVED (embedded `app-update.yml` is the single source of truth — code/yml drift is the bug we're avoiding). `win.artifactName: CRM28-Phone-Setup-${version}.${ext}` is load-bearing: default produces spaced filenames that disagree with `latest.yml`'s hyphenated URL field, causing 404s during fetch. The frontend buttons hardcode `https://crm28.asg.ge/downloads/phone/CRM28-Phone-Setup.exe` — no codebase change per release because the stable URL is invariant; the FILE behind it changes each release. **Do NOT switch the publish provider back to `github`** — repo is going private, GitHub provider does unauthenticated `api.github.com/repos/.../releases/latest` which returns 404 to private repos. **Do NOT add `setFeedURL()` to `auto-updater.ts`** — code-set feed silently overrides embedded YAML. **Do NOT split any of the four upload/refresh/verify steps out of the release script** — that's literally the bug we're guarding against. **Do NOT change the frontend download buttons to a versioned URL** (would require a frontend deploy per softphone release). **Do NOT manually `scp` files to the VM, ever** — bypasses the verification steps. If the script is broken, fix the script; don't work around it. Procedure: `docs/SOFTPHONE_RELEASE_PROCEDURE.md`. Migration: v1.12.3 was published BOTH via the new `pnpm run release` (which uploads to VM AND creates a GitHub release as a one-time bridge) so v1.12.2 operators with the old GitHub-baked feed see v1.12.3 via GitHub before the repo flips private — from v1.12.4 onward, `gh release create` silently skips when the repo is private and the script's VM steps (which are what actually matter) are unaffected.
