...
30. **Softphone auto-update feed lives on the VM, not GitHub releases — `pnpm run release` is the single atomic command that ships a build** (softphone v1.12.3, `crm-phone/scripts/release.{sh,ps1}` + `crm-phone/electron-builder.yml`) — Three flips in three days landed at the right answer: pre-1.12.2 used `provider: generic` + a separate manual SCP step that kept getting skipped → operators stuck on "up to date"; v1.12.2 switched to `provider: github` against the public repo to make `gh release create` the only step → worked but blocked the plan to make the repo private; v1.12.3 reverted to `provider: generic` BUT bundled the SCP into a single `pnpm run release` command that builds + uploads + verifies in one atomic action → SCP can't be skipped because it IS the release step. The `electron-builder.yml` `publish.url` is `https://crm28.asg.ge/downloads/phone`; `setFeedURL()` in `auto-updater.ts` is REMOVED (embedded `app-update.yml` is the single source of truth — code/yml drift is the bug we're avoiding). `win.artifactName: CRM28-Phone-Setup-${version}.${ext}` is load-bearing: default produces spaced filenames that disagree with `latest.yml`'s hyphenated URL field, causing 404s during fetch. **Do NOT switch the publish provider back to `github`** — repo is going private, GitHub provider does unauthenticated `api.github.com/repos/.../releases/latest` which returns 404 to private repos. **Do NOT add `setFeedURL()` to `auto-updater.ts`** — code-set feed silently overrides embedded YAML. **Do NOT split the SCP step out of the release script** — that's literally the bug we're guarding against. The release script's post-upload `curl latest.yml` cross-check catches "uploaded to wrong directory / nginx serves from elsewhere" silently broken cases. Procedure: `docs/SOFTPHONE_RELEASE_PROCEDURE.md`. Migration: v1.12.3 was published BOTH via the new `pnpm run release` (which uploads to VM AND creates a GitHub release as a one-time bridge) so v1.12.2 operators with the old GitHub-baked feed see v1.12.3 via GitHub before the repo flips private — after that release, `--no-github` becomes the default mental model.
