...
30. **Softphone auto-update reads from GitHub releases — `gh release create` is the single action that ships a new build to operators** (softphone v1.12.2, `crm-phone/electron-builder.yml`) — Before v1.12.2, `electron-updater` was configured with `provider: generic, url: https://crm28.asg.ge/downloads/phone`, plus a redundant `autoUpdater.setFeedURL({...})` call in `auto-updater.ts`. Both pointed at a static path on the production VM, served by nginx. Every release required SCPing `*.exe + *.exe.blockmap + latest.yml` to that path AFTER `gh release create` — and that step kept getting skipped, so operators on older versions saw "up to date" forever and the founder had to ask "did you push to the VM?" on every release. v1.12.2 switched `publish.provider` to `github` (`owner: jemiko1, repo: crm-platform, releaseType: release, vPrefixedTagName: true`) and removed the explicit `setFeedURL` override so the embedded `app-update.yml` is the single source of truth. **Do NOT re-add a `setFeedURL()` call in `auto-updater.ts`** — code-set feed silently overrides the embedded YAML, which means a CI/build config change can be defeated by a stale code path. **Do NOT split publish config between `electron-builder.yml` and code.** Repo is public; no auth needed at runtime (`api.github.com` is reachable from any operator machine). Migration: the v1.12.2 release was bridged ONE LAST TIME to the VM path so existing v1.12.1 operators (with the old baked-in feed) saw the upgrade. From v1.12.3 onward, `gh release create` is sufficient. Procedure: `docs/SOFTPHONE_RELEASE_PROCEDURE.md`. The VM nginx path (`crm28.asg.ge/downloads/phone/*`) still works for direct downloads but is no longer the auto-update source — leave it during the migration window, remove from nginx in a follow-up PR after all operators are on >= v1.12.2.
